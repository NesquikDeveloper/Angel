...
class Launcher {
	...
	async launch(callback, progress, server) {
		console.log("[TESTE] server recebido:", server);
		...
		const p = (text, percent) => {
			if(typeof progress === "function") progress({ text, percent });
		};

		let version;
		if(!fs.existsSync(versionJson)) {
			console.log("Downloading version data...");
			p("Downloading version data...", 2);
		...
		let libIndex = 0;
		let totalLibs = version.libraries.filter(l => {
			if(l.name == "org.apache.logging.log4j:log4j-api:2.0-beta9"
					|| l.name == "org.apache.logging.log4j:log4j-core:2.0-beta9"
					|| l.name == "com.google.code.gson:gson:2.2.4") return false;
			return Library.isApplicable(l.rules) && l.downloads.artifact != null;
		}).length;
		...
		for(let library of version.libraries) {
			...
			if(library.downloads.artifact != null) {
				...
				libIndex++;
				if(totalLibs > 0) {
					let libProgress = 5 + (libIndex / totalLibs) * 30;
					p(`Downloading libraries (${libIndex}/${totalLibs})...`, Math.round(libProgress));
				}
			}
		...
		p("Downloading client...", 50);

		await Version.downloadJar(version);

		// ── Atualização do Bosta Client JAR ──
		if(Config.data.autoUpdate) {
			p("Checking for client updates...", 55);
			try {
				await ipcRenderer.invoke("update-client");
			} catch(_) {
				// Falha silenciosa — usa o JAR local existente
			}
		}

		console.log("Downloading JRE...");
		p("Downloading runtime...", 60);
		...
		p("Downloading Discord library...", 85);
		...
		args.push("-Dio.github.angelclient.client.version=" + Utils.version);
		args.push("-Dio.github.angelclient.client.secret=" + secret);
		...
		args.push("-Dio.github.angelclient.client.discord_lib=" + discordNativeLibrary);
		...
		let gameJar = path.join(__dirname, "game/build/libs/game.jar");
		let hasGameJar = fs.existsSync(gameJar);
		if(hasGameJar) {
			classpath += gameJar;
			classpath += classpathSeparator;
		}
		else {
			console.warn("[Launcher] game.jar not found at", gameJar, "- launching without client mod");
		}
		...
		args.push(activeAccount.type === "offline" ? "0" : await this.accountManager.realToken(activeAccount));
		...
		if(hasGameJar) {
			...
			args.push("io.github.angelclient.client.tweak.Tweaker");
		}
		...
		if(server) {
			...
			if(server.includes(":")) {
				...
				serverHost = parts[0];
				serverPort = parts[1] || "25565";
			}
			args.push("--server");
			args.push(serverHost);
			args.push("--port");
			args.push(serverPort);
			console.log("[BostaClient] Conectando ao servidor: " + serverHost + ":" + serverPort);
		}

		// Abre a janela de logs antes de iniciar o jogo
		ipcRenderer.send("game-started");

		console.log("[BostaClient] Args completos: " + args.join(" "));
		...
		process.stdout.on("data", (data) => {
			...
			// Envia cada linha para a janela de logs
			dataString.split("\n").filter(l => l.trim()).forEach(line => {
				ipcRenderer.send("game-log", "[STDOUT] " + line);
			});
		...
		process.stderr.on("data", (data) => {
			...
			// Envia erros para a janela de logs
			dataString.split("\n").filter(l => l.trim()).forEach(line => {
				ipcRenderer.send("game-log", "[STDERR] " + line);
			});
		...
		process.on("exit", (code) => {
			// Notifica a janela de logs que o jogo encerrou
			ipcRenderer.send("game-stopped");
